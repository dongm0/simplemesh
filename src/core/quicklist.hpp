...
template <typename _T> class QuickList;

template <typename _T> struct Node {
  Node<_T> *m_prev;
  Node<_T> *m_next;
  _T m_data;
  Node(const _T &data = _T()) : m_prev(nullptr), m_next(nullptr), m_data(data) {}
};
...
template <class _T> class QuickList {
public:
...
private:
  ...
  Lnode *m_dummy_tail;
  // uint64_t m_size = 0;
  std::unordered_map<_T, Lnode *> m_hash;
...
public:
  ...
  QuickList(const QuickList<_T> &_rhs)
  ...
  QuickList<_T> &operator=(const QuickList<_T> _rhs) {
    if (*this == _rhs) {
      return this;
    }
    clear();
